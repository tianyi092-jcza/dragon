...
- 产品决定（非原版机制）：玩家统一全部据点后继续停留在战略地图，不播放D7END通关过场；信赖归零或玩家势力灭亡的GAME OVER结束视图继续保留。
...
- 分支`main`；战略天气、暴雨、据点火灾/暴动批次已在`ecfa675`提交并推送。当前主线是取消统一天下后的D7END通关过场，提交状态以`git log`为准。
- 最新完整回归：107项MJS、11项Python及`verify_battle_viewport.js`全部通过；当前变更的最新验证见`docs/checkpoint-journal.md`。
- 当前无硬阻塞。已知天气表现差异仅为战略Canvas尚未模拟原版D51F“每地图tile最多5个覆盖层”的硬上限；不影响RNG、事件、生命周期、消息或城损。
