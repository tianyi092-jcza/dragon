...
- `CF2=0..8`，共9次主更新进入下一游戏时刻；`CF3=0..23`后进一天。128军团槽每8次主更新轮完，192据点槽约每日轮完。军团沿路移动必须由其槽位轮到的战略主更新推进，不能绑定每日刷新；大地图只绘军团标识，不绘目标路线虚线。
...
- 新游戏显示地图前执行一次`0x2BD9`，月结再次生成关系/事件候选；读档不得重复开局初始化。候选使用前192城、独立空城sentinel、战争marker及原版不稳定selection-sort；玩家对和平第一候选单向`-8`，对已交战第一候选仅`-1`。
- `0x2E33/0x2E89/0x2EFB/0x2F71`分别生成协同、停战、普通宣战与空城扩张事件；随机入槽不去重且必须保持RNG消费。月结还会按`0x2D8E..0x2DB7`清理失效战略目标。
...
- 首页不播放开场动画；确认新游戏章节/势力/军师或有效存档前，只显示`grf/ui/loginbg.jpg`与标题选单，不装配默认剧本、不请求地图/道路/战斗资源，也不显示GameBar。确认后才加载对应资源并直接进入所选地图。
