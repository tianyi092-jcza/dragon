...
} from "../web/src/game/battle/originalmessages.js";
...
const legion = (faction, slot = faction, generalIdx = faction) => ({
  slot,
  generalIdx,
...
assert.equal(
  resolveOriginalTacticalTalk(catalog, context, 1, 0x1b8).text,
  "還以為是誰！將2啊\n，我就一刀把你砍下來\n！！",
);
// Raw TALK681 skips first stack word and consumes speaker in a later line.
const c3 = structuredClone(context);
c3.speakers[0].personality = 3;
const crossLine = resolveOriginalTacticalTalk(catalog, c3, 0, 0x1b8);
assert.equal(crossLine.index, 681);
assert.equal(crossLine.argumentWordsConsumed, 2);
assert.ok(crossLine.text.includes("將2"));
assert.ok(!crossLine.text.includes("將3"));
const c2 = structuredClone(context);
c2.speakers[0].personality = 2;
assert.ok(
  resolveOriginalTacticalTalk(catalog, c2, 0, 0x1ac).text.includes("將4"),
  "TALK584 uses current advisor, not stack",
);
assert.equal(
  resolveOriginalTacticalTalk(catalog, null, 0, 0x1b7).status,
  "unresolved-slot-context",
);
assert.equal(
  resolveOriginalTacticalTalk(null, context, 0, 0x1b7).status,
  "unresolved-talk-asset",
);
assert.equal(
  resolveOriginalTacticalTalk(catalog, context, 0, 0xffff).status,
  "unresolved-talk-index",
);
assert.equal(
  originalTalkContext(sc, [{ generalIdx: 0, idx: 0 }, legion(1)]).speakers[0],
  null,
);
...
for (const counter of [0x00f0, 0x12f0, 0xffc3]) {
  const s = new OriginalBattleSession({
    ...
    talkCatalog: catalog,
    registers: {
      tacticalFrameCounter: counter,
      side0Active: 1,
      side1Active: 1,
    },
  });
  const rng = s.rng.snapshot();
  s.emitTalk(0, 0x1b7, "fixture-after-A12A");
  s.emitTalk(1, 0x1b8, "fixture-after-A12A");
  ...
  if ((counter & 0xff00) === 0xff00)
    assert.ok(
      s.messages.slots[1],
      "FFFF side0 sentinel keeps intercepting each cycle",
    );
  else assert.equal(s.messages.slots[1], null);
  assert.equal(s.registers.side0MarkerAt, 0xffff);
  assert.equal(s.registers.tacticalFrameCounter & 0xff00, counter & 0xff00);
...
}
{
  ...
  s.emitTalk(0, 0x1b7, "fixture");
  s.emitTalk(1, 0x1b8, "fixture");
  s.tick();
  s.emitTalk(0, 0x1b1, "replacement");
  assert.equal(s.registers.side0MarkerAt, 0x343b);
  assert.equal(s.registers.side1MarkerAt, 0x343a);
  const snapshot = s.snapshot(),
    twin = new OriginalBattleSession({ talkCatalog: catalog });
  twin.restore(snapshot);
  snapshot.messages.slots[0].lines[0] = "poison";
  snapshot.messages.context.speakers[0].name = "poison";
  assert.equal(s.messages.slots[0].text, "擺出陣形！！");
  assert.equal(twin.messages.context.speakers[0].name, "將2");
  for (const target of [s, twin]) {
    assert.equal(target.messageInput(27, 0), null);
    assert.equal(target.messageInput(0, 2), null);
    target.messageInput(28, 2);
    assert.ok(target.messages.slots[0]);
    assert.equal(target.messages.slots[1], null);
    target.tick();
    target.emitTalk(1, 0x1b8, "post-restore");
  }
  assert.deepEqual(s.snapshot(), twin.snapshot());
}

function make() {
  const battle = createFieldBattle(sc, legion(0, 2, 0), legion(1, 3, 1), maps, {
    directoryIndex: 0xc0,
  });
  ...
  for (const [a, x, y] of [
    ...
    s.pool.write16(a, O.SPATIAL_0C, y * 64 + x);
    s.pool.write16(a, O.SPATIAL_0E, y * 64 + x);
  }
...
// and A12A; rejected A7B7 current5 must NOT suppress the input-stage speech.
{
  ...
  assert.equal(s.messages.slots[1].index, 1006); // selected raw block2 starts 1330h, AH19
  assert.equal(s.registers.side0MarkerAt, 0x2c);
  assert.equal(s.registers.side1MarkerAt, 0x2c);
  assert.equal(s.registers.tacticalFrameCounter, 0xf1);
  const frame = s.frame;
  queueTacticalPanelInput(battle, {
    type: "message-input",
    hitId: 28,
    button: 2,
  });
  advanceOriginalScriptFrame(battle, vm);
  assert.equal(s.frame, frame + 1);
  assert.equal(s.messages.slots[1], null);
  assert.ok(s.messages.slots[0]);
  assert.deepEqual(s.rng.snapshot(), rng);
  const snapshot = s.snapshot(),
    copy = make();
  copy.s.restore(snapshot);
  // VM registers are not part of Session snapshot: restore its existing explicit state.
  for (const k of ["pc", "wait", "R", "cmd", "mode", "done"])
    copy.vm[k] = vm[k];
  advanceOriginalScriptFrame(battle, vm);
  advanceOriginalScriptFrame(copy.battle, copy.vm);
...
for (const winner of [0, 1, 2])
  for (const command of [
    ...
    for (const themeFlag of [0, 42])
      for (const battleSideFlag of [0, 0x80]) {
        const { battle, s, vm } = make();
        vm.pc = 1; // original WAIT, not a stubbed VM
        Object.assign(s.registers, {
          winnerState: winner,
          themeFlag,
          battleSideFlag,
          selectedGroupMask: 4,
        ...
        const rng = s.rng.snapshot();
        queueTacticalCommand(battle, { command });
        advanceOriginalScriptFrame(battle, vm);
            ...
            : command === "wall"
              ? themeFlag === 0
                ...
                : battleSideFlag
                  ...
                  : 0x1b4
              : 0x1b1 +
                ["formation", "attack", "assault", "wall", "defend"].indexOf(
                  command,
                );
        assert.equal(
          s.messages.slots[0]?.selector ?? null,
          rejected ? null : selector,
        );
        assert.deepEqual(s.rng.snapshot(), rng);
      }
// A69F all8 cc x4 D349, raw op16 records at all actual sites via production IO.
{
  ...
  for (let cc = 0; cc < 8; cc++)
    for (let winner = 0; winner < 4; winner++) {
      ...
      vm.words = [(20 << 8) | (cc << 5) | 16];
      vm.pc = 0;
      vm.wait = 0;
      s.registers.winnerState = winner;
      s.messages.slots = [null, null];
      vm.step();
      let expected = cc & 6;
      if (expected && cc & 1) expected ^= 6;
      expected >>= 1;
...
}
{
  const { s } = make();
  s.registers.mode = 0;
  s.registers.tacticalFrameCounter = 0x12f0;
  s.pool.write8(0, O.DIRECTION, 1);
  const a = 0xc00;
  s.mapObjects.write8(a, 0, 0x80);
  s.mapObjects.write8(a, 1, 1);
  s.mapObjects.write16(a, 0x18, 0x1001);
  const rng = s.rng.snapshot();
  s.collide(0, 0x61);
  assert.deepEqual(s.messages.wall, {
    label: "門強度",
    hitId: 29,
    address: a,
    value: 151,
  ...
  s.registers.tacticalFrameCounter = 0x12f1;
  s.mapObjects.write16(a, 0x18, 0x1001);
  s.collide(0, 0x61);
  assert.equal(
    s.registers.wallMarkerAt,
    0x1204,
    "equal metric does not extend",
  );
  s.mapObjects.write16(a, 0x18, 0x801);
  s.collide(0, 0x61);
  assert.equal(s.registers.wallMarkerAt, 0x1205);
  assert.equal(s.messages.wall.value, 128);
  assert.equal(s.messageInput(29, 0), null);
  s.messageInput(29, 2);
  assert.equal(s.messages.wall, null);
  assert.deepEqual(s.rng.snapshot(), rng);
  const close = s.events.at(-1);
  assert.equal(close.unmask.height, 4, "C4B7 differs from opening2");
  s.mapObjects.write16(a, 0x18, 1);
  s.collide(0, 0x61);
  assert.equal(
    s.messages.wall.value,
    0,
    "1→0 decrements and displays; no B799 this call",
  );
  // Let the actual A12A expire while making no extra collision calls.
  s.objectsInitialized = false;
  for (let i = 0; i < 20; i++) s.tick();
  assert.equal(s.messages.wall, null);
  assert.equal(s.registers.wallMarkerAt, 0xffff);
}

// Actual raw op3 retreat through VM production IO, both accepting/rejecting gates.
for (const winner of [0, 1, 2]) {
  const { s, vm } = make();
  const block = scripts.find((words) =>
    words.some((w) => (w & 31) === 3 && w >>> 8 === 5),
  );
  ...
  vm.step();
  assert.equal(
    s.messages.slots[1]?.selector ?? null,
    winner === 0 ? 0x1af : null,
  );
  assert.deepEqual(s.rng.snapshot(), rng);
}
// Real ADC8/AF69 -> B533/B5B7/B60F -> C407, after this frame's A12A.
{
  ...
  for (const [field, value] of [
    ...
    s.pool.write8(0, field, value);
  s.pool.write16(0, O.SPATIAL_0C, 66);
  s.pool.write16(0, O.SPATIAL_0E, 66);
  s.pool.write16(0, O.TARGET_X, 0x104);
  s.pool.write16(0, O.POSITION_X, 0x104);
  for (let x = 1; x <= 4; x++)
    s.spatial.write8(0x7000 + 64 + x, (x > 1 ? 16 : 0) | (x < 4 ? 32 : 0));
  s.spatial.write8(67, 0x61);
  s.spatial.write8(0x1043, 0x61);
  s.spatial.writeTile(67, 0xd0);
  s.mapObjects.write8(0xc00, 0, 0x80);
  s.mapObjects.write8(0xc00, 1, 1);
  s.mapObjects.write16(0xc00, 0x18, 500);
  const rng = s.rng.snapshot();
  advanceOriginalScriptFrame(battle, vm);
  assert.equal(s.pool.read8(0, O.ANCHOR_X), 2);
  assert.equal(s.mapObjects.read16(0xc00, 0x18), 499);
  assert.equal(s.messages.wall.value, 31);
  assert.equal(s.registers.wallMarkerAt, 21);
  assert.ok(
    battle.originalLastEvents.some((e) => e.type === "tactical-wall-update"),
  );
  assert.deepEqual(s.rng.snapshot(), rng);
}
...
// C405/captured text; FFFF itself is ambiguous when D319=FF.
{
  ...
  s.mapObjects.write8(0xc00, 0, 0x80);
  s.mapObjects.write8(0xc00, 1, 1);
  s.mapObjects.write16(0xc00, 0x18, 257);
  s.collide(0, 0x61);
  s.emitTalk(0, 0x1b7, "restore-fixture");
  ...
  for (const corrupt of [
    ...
    (x) => {
      delete x.messages.wallMinimum;
    },
    (x) => {
      delete x.messages.catalogHash;
    },
    (x) => {
      x.messages.revision = 2;
    },
    (x) => {
      x.messages.catalogHash = "wrong";
    },
  ]) {
    ...
    assert.deepEqual(
      s.snapshot(),
      before,
      "failed message restore changes no rule/RNG/queue byte",
    );
  }
  const missing = new OriginalBattleSession();
  ...
  }, /invalid-tactical-talk-catalog/);
  const twin = make().s;
  twin.restore(saved);
  assert.equal(twin.messages.wallMinimum, 256);
  twin.registers.tacticalFrameCounter++;
  twin.mapObjects.write16(0xc00, 0x18, 257);
  twin.collide(0, 0x61);
  assert.equal(
    twin.registers.wallMarkerAt,
    saved.registers.wallMarkerAt,
    "restored equal value cannot extend expiry",
  );
  twin.objectsInitialized = false;
  for (let i = 0; i < 19; i++) twin.tick();
  assert.equal(twin.messages.wall, null);
  const legacy = new OriginalBattleSession().snapshot();
  delete legacy.messages;
  assert.doesNotThrow(
    () => new OriginalBattleSession().restore(legacy),
    "closed non-FF-high legacy subset",
  );
...
// facade constructors/attack-defense mapping. No fallback slot invention here.
{
  ...
  const faction = official.factions.find((f) => f.idx === 13);
  faction.target_faction = 0;
  official.diplomacy[13][0] = 0;
  official.diplomacy[0][13] = 0;
  tickStrategicCity(
    {
      scenario: official,
      originalRng: createOriginalBattleRng(),
      gamebar: {
        enqueueTalkMessage() {
          assert.fail("NPC message");
        },
      },
    },
    76,
  );
  const enemy = official.legions[0];
  assert.ok(enemy);
  assert.equal(enemy.slot, enemy.generalIdx);
  const from = official.cities[88],
    target = official.cities[76];
  const dispatched = dispatch(official, from, target);
  assert.ok(dispatched.ok, dispatched.err);
  const player = official.legions.at(-1);
  assert.equal(player.slot, player.generalIdx);
  for (const reverse of [false, true]) {
    const [a, d] = reverse ? [enemy, player] : [player, enemy];
    for (const b of [
      createFieldBattle(official, a, d, maps, { directoryIndex: 0xc0 }),
      createBattle(official, a, target, maps, d),
    ]) {
      b.session.messages.catalog = catalog;
      b.session.emitTalk(0, 0x1b7, "legal-facade");
  ...
  assert.equal(
    originalTalkContext(official, [player, { generalIdx: 127 }]).speakers[1],
    null,
  );
}
console.log(
  "original messages OK: raw indices/slot/stack, 60/20 byte-marker windows, local input, all command/A69F gates, production VM frames/auto retreat/restore/RNG and official formation/facade mappings; UI timing pending owner",
);
