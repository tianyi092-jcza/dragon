...
const match = source.match(/async saveGame\(slotIdx, label\) \{([\s\S]*?)\n {2}\},\n\n {2}\/\*\* 读档/);
...
const commitSaveImage = (dat) => { binaryBase = dat.slice(); };
...
const app = {
  runtimeEnabled: true,
  saves: { slots: [oldSlot] },
  hud: { flashEvent(message) { messages.push(message); } },
...
process.stdout.write("save client commit OK: failed save leaves slots/buffer unchanged; next slot commits\n");
