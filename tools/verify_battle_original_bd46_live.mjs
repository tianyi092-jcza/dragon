...
const {createFieldBattle, advanceOriginalScriptFrame} = await moduleAt('game/tacticalbattle.js');
const {ORIGINAL_OBJECT: O} = await moduleAt('game/battle/originalstate.js');
const {createOriginalPathBuilder} = await moduleAt('game/battle/originalpathfinder.js');
const json = (name) => {
  try {
...
const make = ()=>createFieldBattle(sc,legion(0),legion(1),maps,{directoryIndex:0xc0});
const { BattleView } = await moduleAt('render/battleview.js');
const scripts = json('battle_scripts');
function place(s,a,x,y,plane=0) {
  for(const [k,v] of [[O.FLAGS,0xc0],[O.HP,180],[O.ANCHOR_X,x],[O.PREVIOUS_X,x],
    [O.ANCHOR_Y,y],[O.PREVIOUS_Y,y],[O.CURRENT_COMMAND,8],[O.PENDING_COMMAND,8],
    [O.HEIGHT,plane],[O.PREVIOUS_HEIGHT,plane],[O.LEVEL,plane?2:0],[O.PREVIOUS_LEVEL,plane?2:0],
    [O.POSITION_X,x],[O.POSITION_Y,y]]) s.pool.write8(a,k,v);
...
for(const plane of [0,0x10]) {
  const battle=make(),s=battle.session;
  const notifications=[];
  const view={battle,app:{battleScripts:scripts,hud:{flashEvent(message){notifications.push(message);}}}};
  ...
  s.registers.side0Active=2;s.registers.side1Active=1;s.registers.winnerState=0;s.registers.mode=1;
  for(const p of [0,0x1000]) for(let x=1;x<=4;x++)
    s.spatial.write8(0x7000+p+64+x,(x>1?0x10:0)|(x<4?0x20:0)|(p?2:0));
  ...
  place(s,0,20,20);place(s,0x600,30,30);place(s,0x20,3,1,plane);
  const request={current:0x101,target:0x104,layer:plane,mask:plane?0x74:0xeb,endpointPolicy:plane?0:1};
  const oldInput=createOriginalPathBuilder(Uint8Array.from(s.spatial.navigationBytes()));
  const before=oldInput(request),rng=s.rng.snapshot();
  advanceOriginalScriptFrame(battle,vm);
  assert.equal(s.spatial.read8((plane?0xa000:0x9000)+0x43),8);
  assert.equal(s.pool.read8(0x20,O.STATE),1,'exactly one B240 phase transition');
  const live=battle.originalPathBuilder(request);
  assert.deepEqual(before,{carry:false,words:[0x104],distance:5,visited:4});
  assert.deepEqual(live,{carry:false,words:[0x103,0x104],distance:13,visited:4});
  assert.deepEqual(oldInput(request),before,'old detached navigation does not see B240 writes');

  // Restore copies spatial bytes and keeps facade builder bound to its same buffer.
  const snapshot=s.snapshot(),twin=make();twin.session.restore(snapshot);
  ...
  s.restore(snapshot);assert.deepEqual(battle.originalPathBuilder(request),live);
  assert.notEqual(s.spatial.bytes,twin.session.spatial.bytes);
  assert.notEqual(s.spatial.tileAttributes,twin.session.spatial.tileAttributes);
  ...
  place(s,0,1,1,plane);s.pool.write8(0,O.CLASS,plane?0x24:0);
  ...
  assert.deepEqual(path.built,live);assert.deepEqual(path.first,{carry:false,exhausted:false,word:0x103});
  ...
  assert.equal(s.paths.head,s.paths.tail,'successful reachable request was dequeued');
  assert.deepEqual(s.rng.snapshot(),rng,'new writes/search/queue consume zero RNG');
  ...
  advanceOriginalScriptFrame(battle,vm);
  assert.equal(s.pool.read8(0,O.PATH_REMAINING),0,'B00D exhausted clamps zero');
  assert.equal(s.pool.read8(0,O.PATH_OFFSET),4,'exhausted does not read another word');
  ...
  console.log(`BD46 live plane ${plane}: CF0 [0103,0104], AED2 first=0103, AF65 same-frame second consumption, snapshot/RNG invariant`);
}
