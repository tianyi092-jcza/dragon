...
for (const counter of [0x2b, 0xfffe]) {
  ...
  for (const a of [next, 0x11b, 0x11c, 0xffff])
    for (const b of [next, 0x11b, 0x11c, 0xffff])
      for (const c of [next, 0x11b, 0x11c, 0xffff])
        fixtures.push({ counter, markers: [a, b, c] });
}
...
const result = spawnSync(
  "python",
  ["-B", "tools/tactical_message_raw_oracle.py"],
  {
...
for (let i = 0; i < fixtures.length; i++) {
  ...
  const s = new OriginalBattleSession({
    registers: {
      tacticalFrameCounter: f.counter,
      side0MarkerAt: f.markers[0],
      side1MarkerAt: f.markers[1],
      wallMarkerAt: f.markers[2],
      side0Active: 1,
      side1Active: 1,
    },
  });
  ...
  assert.deepEqual(
    [
      s.registers.side0MarkerAt,
      s.registers.side1MarkerAt,
      s.registers.wallMarkerAt,
    ],
    oracle.markers,
    JSON.stringify(f),
  );
  const closes = frame.events
    .filter((e) => e.type === "side-marker" || e.type === "wall-marker")
    .map((e) => (e.type === "wall-marker" ? 2 : e.side));
  assert.deepEqual(closes, oracle.closes, JSON.stringify(f));
  assert.deepEqual(s.rng.snapshot(), rng);
}
const same = raw[0];
assert.deepEqual(same.closes, [0]);
assert.deepEqual(same.trace, [
  [0xa133, 0x2c],
  [0xa13f, 0x11b],
  [0xa14b, 0x11b],
  [0xa155, 0x11b],
]);
const cascade = fixtures.findIndex(
  (f) =>
    f.counter === 0x2b &&
    f.markers[0] === 0x2c &&
    f.markers[1] === 0x11b &&
...
assert.deepEqual(raw[cascade].trace, [
  [0xa133, 0x2c],
  [0xa13f, 0x11b],
  [0xa14b, 0x11c],
  [0xa155, 0x11c],
...
console.log(
  `message raw AX OK: ${fixtures.length} no-stub bounded KI comparisons, same-deadline suppression/011B-011C cascade/wall/FFFF/lowbyte and deferred restore continuation`,
);
