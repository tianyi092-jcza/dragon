...
权重语义已由构图探针闭合：edge `weight`等于原版道路点列长度；全36672个有序据点对的转录搜索与独立Dijkstra累计代价0 mismatch。未发现额外地形/江河乘数；势力通行只在候选阻断/后处理层生效，不进入静态edge weight。
...
野外接敌 `0x2831` 与攻城检测 `0x2880` 共享同一预战阶段：
