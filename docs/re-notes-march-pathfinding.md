...
待验证：节点 `+4` 的权重是否纯粹为道路长度，是否额外包含地形、江河或势力通行成本。
...
8. 玩家战术攻城退出已接入`0xA65D→0x9FF8`城损聚合纯函数：从16条`0xC00`城壁对象中取`kind==1`的最小`+0x18`，若全部bit0仍置位则乘4，再以`(城兵+50-floor(metric/10))>>3`同步扣城兵、上升率、防灾。当前战术引擎尚未移植`0x9B40`初始化及城壁受击对象状态机，故没有真实wallRecords时明确不写城损；不再以`defLeft`、战略ratio或臆造metric冒充原版公式；
