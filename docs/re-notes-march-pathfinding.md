...
7. 二进制 SAVE.DAT 仍只写当前坐标，不复刻途中 edge/stride/point 指针；即时 Web 快照会剔除导航缓存并在读入后重建。`0x4DA4`字段已闭合为`+0x20=目标城、+0x14=目标节点、+0x0B=1、status|=2`；Web额外的`commandState=8`仅为调度兼容，不宣称是该函数写入；
