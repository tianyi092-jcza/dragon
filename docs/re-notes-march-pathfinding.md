...
Web直接使用四幅原始动画资源，不再使用单振荡器扫频近似。2026-09-05现场复核先发现WAV有效声段约330ms，因而每声以2倍速播放完整约165ms包络；同日用户进一步以原版实机校正为一次接战共五声、起拍间隔约82.5ms。因此`engageSfxBurst()`在接触建立时把五声一次性排入同一WebAudio时间轴，相邻样本允许重叠；音频`resume/statechange`不得阻塞首帧。五声与82.5ms是实机观察支持的Web表现标定；KI静态代码中的ID3调用次数还受军团`+0x1E`动作间隔及YNSOUND busy门影响，尚不能推广为所有接触场景的固定机制。

绘制位置由静态指令闭合：`0x2AF4→0x2B3C→0xD51F`先读取攻方军团`+0x10/+0x12`画48×48接战图；`0x2AF4→0x2B2A→0xD4C7`随后读取同一坐标画16×16军团标识。故动画与标识都停在城前军团坐标，且标识压在动画之上；旧Web“目标据点中心”定位已撤销。规则倒计时本身从接触当轮立即选择图帧，不能在倒计时结束后再追加一轮专用过渡。四张PNG调度保留；GIF不能解决启动等待，且不利于帧重置和与声音共用确定时基。
...
4. 野外道路段军团相遇、接敌倒计时和攻城等待动画已接入：进入下一道路点前先判占用；候选格为末端`0xCE..0xDD`据点边界tile时再按edge端点城主判攻城，发起军团均停在原点并建立11→1接触状态，防守军团不被同步置状态。接触建立当轮即按规则倒计时低两位绘图并预排五声；结算只用单RAF gate串行化，不再等待音频或追加专用四相；野战胜方在下一次自身槽立即续行，不追加Web冷却；
...
5. **接入原版标识与接战动画**
   ...
   - 已从 `MMAP.MCH + 0xA000` 的 `0x2B3C` 描述表精确合成 group 0 四相48×48动画（phase 1/3资源相同）；规则态按倒计时低两位从接触当轮直接投影，动画和攻方标识共用城前军团坐标，动画先画、16×16标识后画；
   - 已接入名义12、同轮减为11后逐调度递减的接触状态；`engageSfxBurst()`将按SOUND.DAT ID3→ID13寄存器链渲染并经用户听感确认的`web/grf/sfx/ynsound-id3.wav`以2倍速播放完整165ms包络，并按用户原版实机复核在同一WebAudio时钟预排五声、间隔82.5ms；音频准备不阻塞首帧；
