...
因此 Web 当前“从地图图块提取可走格，再在 384×256 网格上做 8 向 A*”只能算视觉近似，不是原版的数据模型。
...
Web委任过渡的四幅动画采用逐相同步播放该样本，每相约550ms以容纳ID3完整3+7 tick包络，不再使用单振荡器扫频近似。需要区分：四相四声是用户现场听感确认后的表现同步；KI静态代码中的ID3调用次数还受军团`+0x1E`动作间隔及YNSOUND busy门影响，尚不能推广为所有接触场景固定四次。

野外接敌 `0x2831` 与攻城检测 `0x2880` 共享同一预战阶段。`0x2736`先调用`0x2831`；若下一点已有敌军团，CLC后主调度立即返回，不会继续执行`0x2880`。所以据点中心有真实驻军时先走`0x4A7B`军团战，只有无敌军团时才走`0x4ADE`据点攻城：
...
- `web/src/game/ai.js`：据点目标沿当前道路边点列每日推进一格，到边端下一日重新寻路；
...
5. **接入原版标识与接战动画**
   ...
   - 已接入名义12、同轮减为11后逐调度递减的接触状态，并新增受忙状态门控的 `engageSfx()` 近似 YNSOUND ID 3；真实ID3音色仍待解码；
...
6. **再连接战斗触发**
   - 只有路线、接敌检测、到城判定和失败回退全部稳定后，再进入野战/攻城战底层机制。
